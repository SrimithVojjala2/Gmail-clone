...
<template>
  <div>
    <messageComp v-for="(item, index) in list" :id="item.id" :key="item.id" :from="item.title" :subject="item.category"
      :description="item.description" :time="item.rating" :markValue="markValue" @removeMessage="removeMessage(index)"
      @updatecheckboxlist="(test) => addcheckboxlist(index, test)"
      @sendchecklist="(checklist) => sendchecklist(index, checklist)" @changeMessageComp="changeMessageComp" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    changeMessageComp(value) {
      this.$emit("changeMessageComp", [value, 'promotions']);
    },
  },
  watch: {
